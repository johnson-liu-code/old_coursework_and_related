...
int main()
{
    ...
    promotion_specs *array_1 = new promotion_specs[1];
    promotion_specs *array_2 = new promotion_specs[1];
    ...
    while (command != "QUIT")
    {
        ...
        if (command == "ADD" || command == "CHANGE")
        {
            ...
            if (command == "ADD")
            {
                if (first_promotion == true)
                {
                    array_1[0] = promotion_specs(promotion, from, to, airline, flight_price, number_of_nights, price_per_night);
                    first_promotion = false;
                }
                else
                {
                    array_size++;
                    promotion_specs *array_2 = new promotion_specs[array_size];
                    for (int i = 0; i < array_size - 1; i++)
                    {
                        array_2[i] = array_1[i];
                    }
                    array_2[array_size-1] = promotion_specs(promotion, from, to, airline, flight_price, number_of_nights, price_per_night);
                    promotion_specs *array_1 = new promotion_specs[array_size];
...
*/
    ...
    // for (int i = 0; i < array_size; i++)
    // {
    //     cout << array_1[i].get_promotion() << endl;
    // }
