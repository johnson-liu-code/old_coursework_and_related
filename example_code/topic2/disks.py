...
class System:
    ...
    def pair_histogram(self, K=1.5, N_bins=64):
        histogram = [ 0 ] * N_bins
        DeltaA2 = (K**2 - 1) * math.pi * self.d_0**2 / N_bins
...
response = input(" Run animation? yes or no: ")
if len(response) == 0 or response[0].lower() == 'y':
    ...
    for step in range(mc_cycles):
        ...
        system.cycle()
    with open("disks.data", "w") as file:
        for bin in range(N_bins):
            m = bin + 1
            file.write(str(m) + "\t" + str(histogram[bin]) + "\n")
    print(" Histogram written to file disks.data")
