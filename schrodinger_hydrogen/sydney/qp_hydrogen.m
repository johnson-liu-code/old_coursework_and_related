...
% hold on;
% plot(psi(:,3), 'LineWidth', 2, 'color', 'blue')
...
% set(gca,'fontsize',12')
% plot(r,U_c,'b','linewidth',2);
% hold on;
% plot(r,U_L,'r','linewidth',2);
% plot(r,U,'k','linewidth',2);
% legend('coul', 'Ang Mon', 'eff');
% grid on;
% axis([r_min r_max -5*E_max 5*E_max]);
...
% hold on
% for cn = 1 : n
% h_plot = plot([r_min r_max], [E(cn) E(cn)]);
% set(h_plot, 'lineWidth',0.1, 'color', [1 0 0]);
% end
% axis([r_min 4*r_peak -5*E_max 10]);
% 
...
% 
% subplot(1,2,1)
%    if psi(3,qn) > 0 sf = 1; else sf = -1; end;
%    plot(r,sf .* psi(:,qn),'LineWidth',2);
%    grid on; hold on
%    plot([r_peak r_peak],[0, max(sf .* psi(:,qn))],'r','linewidth',2)
%    tm1 = 'r_{peak} =  ';
%    tm2 = num2str(r_peak,4);
%    tm3 = '  m';
%    tm = [tm1 tm2 tm3];
%    h_title = title(tm);
%    set(h_title,'color',[1 0 0],'fontsize',fs);
%    ylabel('wave function,   \psi ','fontsize',fs);
%    xlabel('position x (m)','fontsize',fs);
%    axis([r_min 4*r_peak -1.1*max(abs(psi(:,qn))) 1.1*max(abs(psi(:,qn)))]);
%    set(gca,'fontsize',fs);
%    set(gca,'Position',[0.066 0.12 0.38 0.71])
% 
%    subplot(1,2,2);
%    plot(r,prob(:,qn),'LineWidth',2);
%    grid on
%    xlabel('position x (m)','fontsize',fs);
%    ylabel('prob. density,  \psi^2 (m^{-1}) ','fontsize',14);
%    hold on
%    plot([r_peak r_peak],[0, max(Prob_density)],'r','linewidth',2)
%    plot([ravg ravg],[0, max(Prob_density)],'m','linewidth',2)
%    tm1 = 'r_{avg} =  ';
%    tm2 = num2str(ravg);
%    tm3 = '  m';
%    tm = [tm1 tm2 tm3];
%    title(tm);
%    h_title = title(tm);
%    set(h_title,'color',[1 0 1],'fontsize',fs);
%    axis([r_min 4*r_peak 0 1.1*max(abs(prob(:,qn)))]);
%    set(gca,'fontsize',fs);
%    set(gca,'Position',[0.55 0.12 0.38 0.71])
%    
% % Graphical output for <x>
% figure(4)
% set(gcf,'Name','<r>','NumberTitle','off')
% set(gcf,'Units','normalized','Position',[0.3 0.3 0.6 0.6]);
% fs = 12; set(gca,'fontsize',fs);
% subplot(3,1,1)
%    plot(r,-Psi,'LineWidth',2);
%    grid on
...
%    
% subplot(3,1,2)
%    plot(r,ket,'LineWidth',2);
%    grid on
%    ylabel('ket ','fontsize',fs);
%    axis([r_min 4*r_peak -1.1*max(abs(ket)) 1.1*max(abs(ket))]);
%    set(gca,'fontsize',fs)
%    
% subplot(3,1,3);
%    plot(r,braket);
...
%    xlabel('position r (m)','fontsize',fs);
%    hold off
%    axis([r_min 4*r_peak -1.1*max(abs(braket)) 1.1*max(abs(braket))]);
%    set(gca,'fontsize',fs)
%    
%    
% Probability Clouds ++++++++++++++++++++++++++++++++++++++++++++++++++++++
